...
def get_study_level_data(study_type):
    ...
    for phase in data_cat:
        BASE_DIR = '/home/joshua/mura-data/MURA-v1.1/{}/{}/'.format(phase, study_type)
        print("Loading {} data in {}".format(phase, BASE_DIR))
        patients = list(os.walk(BASE_DIR))[0][1] # list of patient folder names
        study_data[phase] = pd.DataFrame(columns=['Path', 'Count', 'Label'])
        i = 0
        patient_loader = tqdm(patients, desc="Loading Patients")
        for patient in patient_loader:
            patient_loader.set_description("Loading Patients [{}]".format(patient))
            for study in os.listdir(BASE_DIR + patient): # for each study in that patient folder
                ...
                count = list(filter(lambda file: not file.startswith("."), os.listdir(path)))
                study_data[phase].loc[i] = [path, len(count), label] # add new row
                i+=1
    return study_data
